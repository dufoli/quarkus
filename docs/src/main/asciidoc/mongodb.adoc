...
== Multiple MongoDB Connection

MongoDB allows you to configure multiple connection.
It works exactly the same way as a single connection, with one important change: a name.
...
When using multiple connection, each `MongoClient` also has the `io.quarkus.mongodb.runtime.Connection` qualifier with the name of the connection in the property as the value.
